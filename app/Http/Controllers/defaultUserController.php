...
use App\Customer;
use Notification;
use App\Notifications\PlanPaid;
class defaultUserController extends Controller
{
    ...
    public function postSignup(Request $request){
    	...
    	if ($customer==true) {
            $adminid=DB::table('customers')->where('role_id','=',1)->pluck('id');
          foreach ($adminid as $key => $id) {
              $myid=$id;
          }
         $admin=Customer::where('id',$myid)->first();
  
            Notification::send($admin, new PlanPaid($customer));
