...
<div class="myfrm">
<h2 class="bg-light p-3">Buy bundles</h2>
@foreach($plan as $p)
<form id="buybundleform">
	<div class="err"></div>
<div class="jumbotron">
  <h1 class="display-4">{{ $p->plantitle }} /Kes {{ $p->cost }}</h1>
  <input type="hidden" name="plan" value="{{ $p->planname }}">
  <input type="hidden" name="amount" value="{{ $p->cost }}">
  <hr class="my-4">
  @if(isset(Auth::user()->phone))
  <input type="text" name="phone" class="form-control p-4" value="{{ Auth::user()->phone }}">
  @else
